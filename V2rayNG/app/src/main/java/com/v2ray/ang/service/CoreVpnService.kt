...
class CoreVpnService : VpnService(), ServiceControl {
    ...
    override fun onCreate() {
        ...
        try {
            ...
            wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "v2rdns:CoreVpnWakeLock").apply {
                acquire()
            }
            LogUtil.i(AppConfig.TAG, "StartCore-VPN: WakeLock acquired")
        } catch (e: Exception) {
            LogUtil.e(AppConfig.TAG, "StartCore-VPN: Failed to acquire WakeLock", e)
        }
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                if (portOpen) {
                    if (isVkTurn) {
                        ...
                        for (j in 0..180) { // wait up to 90 seconds (allows manual captcha solving if triggered)
                            ...
                            if (!isRunning) {
                                LogUtil.w(AppConfig.TAG, "StartCore-VPN: VK TURN client stopped while waiting for streams")
                                break
                            }
                            delay(500)
                        }
                        if (!vkConnected) {
                            LogUtil.e(AppConfig.TAG, "StartCore-VPN: VK TURN tunnel failed to connect (0 streams after timeout)!")
                            withContext(Dispatchers.Main) {
                                stopAllService()
                            }
                            return@launch
